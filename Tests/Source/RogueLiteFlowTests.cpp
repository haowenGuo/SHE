...
bool TestRogueLiteRespondsToGameplayCommands()
{
    she::ApplicationConfig config;
    config.applicationName = "RogueLite Flow Tests";
    ...
    she::RogueLiteFeatureLayer layer;
    layer.OnAttach(services);

    services.gameplay->BeginFrame(1);
    services.gameplay->FlushCommands();

    const she::RogueLiteSnapshot initialSnapshot = layer.BuildSnapshot();

    services.gameplay->QueueCommand("RogueLiteFireRight", "source=test");
    services.gameplay->FlushCommands();
    const she::RogueLiteSnapshot firedSnapshot = layer.BuildSnapshot();

    services.gameplay->QueueCommand("RogueLiteDash", "source=test_dash");
    services.gameplay->FlushCommands();
    const she::RogueLiteSnapshot dashedSnapshot = layer.BuildSnapshot();

    services.gameplay->QueueCommand("RestartRogueLiteRun", "source=test_restart");
    services.gameplay->FlushCommands();
    const she::RogueLiteSnapshot restartedSnapshot = layer.BuildSnapshot();

    bool passed = true;
    passed &= Expect(
        firedSnapshot.activeProjectileCount > initialSnapshot.activeProjectileCount,
        "Fire command should spawn a projectile.");
    passed &= Expect(
        dashedSnapshot.playerPosition.y > firedSnapshot.playerPosition.y,
        "Dash command should move the player.");
    passed &= Expect(
        restartedSnapshot.phase == she::RogueLiteGamePhase::Combat,
        "Restart command should restore the combat state.");
    passed &= Expect(restartedSnapshot.activeProjectileCount == 0, "Restart should clear projectiles.");
    passed &= Expect(restartedSnapshot.roomIndex == 0, "Restart should return to room one.");
    passed &= Expect(
        restartedSnapshot.playerHitPoints == restartedSnapshot.playerMaxHitPoints,
        "Restart should restore full health.");
...
int main(int, char**)
{
    return TestRogueLiteRespondsToGameplayCommands() ? 0 : 1;
}
